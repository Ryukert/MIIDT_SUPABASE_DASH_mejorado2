...
const ui = {
  ...
  dbStatusHero: $id("dbStatusHero"),
  rtStatus: $id("rtStatus"),
  sourceBadge: $id("sourceBadge"),
  sessionBadge: $id("sessionBadge"),
  totalSamples: $id("totalSamples"),
  totalSamplesMini: $id("totalSamplesMini"),
  pcSamples: $id("pcSamples"),
  rpiSamples: $id("rpiSamples"),
  lastSeen: $id("lastSeen"),
  activeDevice: $id("activeDevice"),
  activeSensor: $id("activeSensor"),
...
let decCounter = 0;
const overview = { total: 0, PC: 0, RPI: 0, lastSeen: '-', activeDevice: '-', activeSensor: '-' };
...
function setDb(on) {
  ...
  ui.dbStatusHero.textContent = on ? "Conectado" : "Desconectado";
...
function updateOverviewUI(){
  ui.totalSamples.textContent = String(overview.total);
  ui.totalSamplesMini.textContent = String(overview.total);
  ui.pcSamples.textContent = String(overview.PC);
  ui.rpiSamples.textContent = String(overview.RPI);
  ui.lastSeen.textContent = overview.lastSeen;
  ui.activeDevice.textContent = overview.activeDevice;
  ui.activeSensor.textContent = overview.activeSensor;
}
...
function resetOverview(){
  ...
  overview.RPI = 0;
  overview.lastSeen = '-';
  overview.activeDevice = '-';
  overview.activeSensor = '-';
  updateOverviewUI();
}
...
function ingestRow(row) {
  ...
  overview.total += 1;
  overview[devKey] += 1;
  overview.lastSeen = labelLocal;
  overview.activeDevice = row.device_id || devKey;
  overview.activeSensor = sensorType;
  updateOverviewUI();
...
async function connectSelected() {
  ...
  applySourceUI(src);
  ui.sourceBadge.textContent = src === 'firebase' ? 'Firebase RTDB' : 'Supabase';
  ui.sessionBadge.textContent = ui.sessionId.value.trim() || 'Sin filtro';
...
(function boot() {
  ...
  applySourceUI(src);
  ui.sourceBadge.textContent = src === 'firebase' ? 'Firebase RTDB' : 'Supabase';
  ui.sessionBadge.textContent = 'Sin filtro';
  ...
  ui.dbSource?.addEventListener('change', () => {
    ...
    applySourceUI(v);
    ui.sourceBadge.textContent = v === 'firebase' ? 'Firebase RTDB' : 'Supabase';
  });
})();
